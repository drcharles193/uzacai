...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <header className="w-full py-4 px-6 md:px-10 border-b border-border/40 sticky top-0 backdrop-blur-md bg-background/80 z-50">
        <div className="container max-w-7xl mx-auto flex justify-between items-center animate-fade-in">
          ...
          <div className="flex items-center gap-3">
            <button className="px-3 py-1.5 text-sm font-medium rounded-full border border-border bg-background/80 hover:bg-secondary/80 transition-colors duration-200 focus-ring">
              Sign in
            </button>
            <button className="px-4 py-1.5 text-sm font-medium rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-colors duration-200 shadow-sm focus-ring">
              Get started
            </button>
          </div>
        </div>
      ...
      <footer className="w-full py-8 px-6 md:px-10 border-t border-border/40 bg-secondary/50">
