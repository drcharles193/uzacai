...
const SocialMediaConnect: React.FC<SocialMediaConnectProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleFacebookCallback = (event: MessageEvent) => {
      if (event.data && event.data.type === 'facebook-oauth-callback') {
        const { code, state, error, errorDescription } = event.data;
        console.log('Received Facebook callback:', code ? 'Code received' : 'No code', state ? 'State received' : 'No state');
    ...
    const handleInstagramCallback = (event: MessageEvent) => {
      if (event.data && event.data.type === 'instagram-oauth-callback') {
        const { code, state, error, errorDescription } = event.data;
        console.log('Received Instagram callback:', code ? 'Code received' : 'No code', state ? 'State received' : 'No state');
  ...
  const completeInstagramConnection = async (code: string, state: string) => {
    try {
      ...
      console.log("Instagram callback response:", response);
  ...
  const connectPlatform = async (id: string) => {
    try {
      ...
      // For other platforms (mock connections)
      const response = await supabase.functions.invoke('social-auth', {
        body: JSON.stringify({
          platform: id,
          userId: session.user.id
